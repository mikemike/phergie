...
/**
 * Saves URLs to a remote (or local) database.  Modified from Phergie_Plugin_Url.
 * Uses a local cache to not regrab titles over and over.  Grabs the titles of the
 * pages to also be saved.
 *
 * Usage:
 *   Just mention a URL in any channel the bot is in
 *
 * @category Phergie
 * @package  Phergie_Plugin_SaveUrls
 * @author   Mike Griffiths (http://www.mike-griffiths.co.uk)
 * @license  http://phergie.org/license New BSD License
 ...
 */
class Phergie_Plugin_SaveUrls extends Phergie_Plugin_Abstract
{
    /**
    ...
    /**
     * Checks a given URL against the cache to verify if they were
     ...
     * @return bool
     */
    protected function checkUrlCache($url)
    {
        $cache = array();
        $source = $this->getEvent()->getSource();
         ...
         */
        if ($expire > 0
            && isset($cache['url'][$source][$url])
        ) {
            unset($cache, $url, $expire);
            return true;
        }
        unset($cache, $url, $expire);
        return false;
     ...
     * @return bool
     */
    protected function updateUrlCache($url)
    {
        $cache = array();
        $source = $this->getEvent()->getSource();
